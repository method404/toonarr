...
export async function POST(_request: Request, context: RouteContext) {
  try {
    ...
    if (
      ...
      return NextResponse.json({ error: "Invalid episode retry payload." }, { status: 400 });
    }

    const entry = await retrySeriesEpisodeInLibrary(titleId, no);
    ...
    const message =
      error instanceof Error ? error.message : "Failed to retry episode.";
